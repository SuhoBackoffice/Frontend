...
import {
  getProjectBranchCapacityDetail,
  getProjectBranchCapacityAnalyzeSortTypes,
...
// ── 메인 컴포넌트 ────────────────────────────────────────────────────────────
interface BranchCapacityDetailDialogProps {
  projectId: number;
...
export function BranchCapacityDetailDialog({
  projectId,
  projectBranchId,
  branchName,
  isOpen,
  onClose,
}: BranchCapacityDetailDialogProps) {
  ...
  const [detail, setDetail] = useState<GetBranchCapacityDetailResponse | null>(null);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    if (!isOpen) return;
    getProjectBranchCapacityAnalyzeSortTypes().then((res) => {
  ...
  const fetchDetail = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const res = await getProjectBranchCapacityDetail({
        projectId,
        projectBranchId,
        sort,
        dir,
  ...
  }, [isOpen, projectId, projectBranchId, sort, dir, onlyShortage]);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="!flex !max-h-[90vh] !w-[95vw] !max-w-5xl min-w-[600px] !flex-col gap-0 overflow-hidden p-0 sm:!w-[90vw]">
        {/* ── 헤더 (고정) ── */}
        <DialogHeader className="shrink-0 border-b px-6 py-5">
          <DialogTitle className="gap-2">
            <span className="text-2xl font-bold">{branchName}</span>
            {detail && <span className="text-muted-foreground mt-1 text-sm"> {detail.serial}</span>}
          </DialogTitle>
          <DialogDescription className="text-muted-foreground pt-1 text-sm">
            {detail && <span>현재 {detail!.code}번 분기의 가용 생산 능력 분석 입니다.</span>}
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  >
                    ...
                    <span className="text-muted-foreground ml-1.5 text-base font-normal">SET</span>
                  </p>
                </div>
              </div>
            ) : null}
                        ...
                        >
                          ...
                          <TableCell className="whitespace-nowrap">
                            <div className="flex items-center gap-1.5">
                              ...
                              {item.suppliedMaterial && (
                                <Badge
                                  variant="outline"
                                  className="h-4 border-violet-300 px-1 text-[9px] text-violet-600 dark:border-violet-700 dark:text-violet-400"
                                >
                                  사급
                                </Badge>
                              )}
